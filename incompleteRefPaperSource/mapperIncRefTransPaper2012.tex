...
\cite{Ye2011}.  However, the computational pipeline to process RNA-seq data was developed 
for model organisms with complete or mostly complete reference transcriptomes.  In this study, 
...
transcriptome on per-read accuracy.  We next determine the error in transcript expression 
...
with its originating transcript.  To simulate Illumina reads, single end reads were 100 bp and 
...
the per-read errors lead to more transcripts with inaccurate expression levels reported, with higher 
...
the per-read error.  As seen in Figure \ref{fig:longSeReads}, 20\% more reads are unique when read 
